...
object KVector3DFunc {
    ...
    fun exp(that: KVector3D) = exp(that.scalar) * exp(that.vec) * exp(that.biVec) * exp(that.triVec)
    ...
    fun Double.pow(that: KVector3D) = exp(that * ln(this))
    ...
    fun tanh(that: KVector3D) = sinh(that) / cosh(that)
    ...
    fun cos(that: KVector3D, basis: BiVector3D) =
    ...
    fun cos(that: KVector3D, basis: TriVector3D) =
        sin(that.scalar) * sin(that.vec, basis.norm()) * sin(that.biVec, basis.norm()) * sin(that.triVec, basis.norm()) +
        cos(that.scalar) * cos(that.vec, basis.norm()) * cos(that.biVec, basis.norm()) * cos(that.triVec, basis.norm()) -
        sin(that.scalar) * sin(that.vec, basis.norm()) * cos(that.biVec, basis.norm()) * cos(that.triVec, basis.norm()) -
        sin(that.scalar) * cos(that.vec, basis.norm()) * sin(that.biVec, basis.norm()) * cos(that.triVec, basis.norm()) -
    ...
    fun sin(that: KVector3D, basis: BiVector3D) =
    ...
    fun sin(that: KVector3D, basis: TriVector3D) =
        sin(that.scalar) * cos(that.vec, basis.norm()) * cos(that.biVec, basis.norm()) * cos(that.triVec, basis.norm()) -
        sin(that.scalar) * sin(that.vec, basis.norm()) * sin(that.biVec, basis.norm()) * cos(that.triVec, basis.norm()) -
        sin(that.scalar) * sin(that.vec, basis.norm()) * cos(that.biVec, basis.norm()) * sin(that.triVec, basis.norm()) -
        sin(that.scalar) * cos(that.vec, basis.norm()) * sin(that.biVec, basis.norm()) * sin(that.triVec, basis.norm()) +
    ...
    fun tan(that: KVector3D, basis: BiVector3D) = sin(that, basis) / cos(that, basis)
    fun tan(that: KVector3D, basis: TriVector3D) = sin(that, basis) / cos(that, basis)

    fun ln(that: Double, basis: BiVector3D): KVector3D {
        return if (that >= 0) {
            KVector3D( scalar = ln(that) )
        } else {
            ln(abs(that)) + PI * basis.norm()
        }
    }
    fun ln(that: Double, basis: TriVector3D): KVector3D {
        return if (that >= 0) {
            KVector3D( scalar = ln(that) )
        } else {
            ln(abs(that)) + PI * basis.norm()
    ...
    fun ln(that: Vector3D) = ln(that.magnitude()) - 2
    fun ln(that: BiVector3D): KVector3D = ln(that.magnitude(), that) + (PI / 2) * that.norm()
    fun ln(that: TriVector3D): KVector3D = ln(that.magnitude(), that) + (PI / 2) * that.norm()
}